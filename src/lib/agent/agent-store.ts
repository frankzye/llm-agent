...
export type AgentConfig = {
  ...
  createdAt: string;
  updatedAt: string;
  /** Short human-readable summary for find_agents / A2A (not the model system prompt). */
  description?: string;
...
export function agentsRoot(cwd = process.cwd()): string {
  return path.join(dataRootDir(cwd), "agents");
}
...
/** All agents on disk (one folder per id under the data root). Newest `createdAt` first. */
