 ...
 * (or repo root if no subpath) **directly** into `skillsRoot`.
 * Existing entries with the same name are replaced; other files and folders (including
 * `from-folder/`) are left unchanged.
 */
export async function addSkillsFromGitIntoSkillsRoot(
  skillsRoot: string,
  gitUrl: string,
  opts?: { branch?: string; subpath?: string | null },
...
/** Add or override global skills from Git under the data-root `skills/` directory. */
export async function addGlobalSkillsFromGit(
  cwd: string,
  gitUrl: string,
  opts?: { branch?: string; subpath?: string | null },
): Promise<{ ok: true; path: string }> {
  ...
  await fs.mkdir(skillsRoot, { recursive: true });
  return addSkillsFromGitIntoSkillsRoot(skillsRoot, gitUrl, opts);
}
