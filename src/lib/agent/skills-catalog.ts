...
/**
 * Removes catalog skills by id: deletes each skill’s folder (parent of `SKILL.md`), or the
 * lone `SKILL.md` if it sits at the skills root. Regenerates `skills.json`.
 */
export async function deleteSkillsFromCatalog(
  skillsRoot: string,
  skillIds: string[],
): Promise<{ deleted: string[]; notFound: string[] }> {
  const absRoot = path.resolve(skillsRoot);
  let catalog = await readSkillsCatalog(absRoot);
  if (!catalog) {
    await regenerateSkillsIndex(absRoot);
    catalog = await readSkillsCatalog(absRoot);
  }
  if (!catalog) {
    throw new Error("Could not read or build skills catalog.");
  }
  ...
  for (const rawId of skillIds) {
    ...
    const fullMd = path.join(absRoot, entry.path);
    if (!isPathInside(absRoot, fullMd)) {
      notFound.push(id);
      continue;
    }

    const dir = path.dirname(fullMd);
    try {
      if (path.resolve(dir) === absRoot) {
        await fs.rm(fullMd, { force: true });
      } else {
        if (!isPathInside(absRoot, dir)) {
          notFound.push(id);
          continue;
        }
